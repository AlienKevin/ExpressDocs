function initGlobalVars() {
  PropertiesService.getScriptProperties().setProperty('source', '3');
}

function markdownToDocs() {
  // initialize global variables
  initGlobalVars();
  // initialize AsciiMath to LaTeX converter
  initConvert();

  // Insert a web image  
  // var baseUrl = "http://www.texrendr.com/?eqn=";
  // baseUrl = "https://latex.codecogs.com/gif.latex?\\dpi{200}&space;\\huge&space;";
  ...
  // insert expression image
  replaceDeliminators(body, "\\$\\$", undefined, true, generateExpressionImage);
...
function generateExpressionImage(match, regex) {
  ...
  if (/\\/.test(content)) {
    ...
    // content in asciimath
    var expr = asciimath2latex(content);
  }
  Logger.log("expr: " + expr);
  var url = retrieveImageUrl(expr, 3);
  var image = UrlFetchApp.fetch(url).getBlob();
  return image;
}

function retrieveImageUrl(expr) {
  var source = Number(PropertiesService.getScriptProperties().getProperty('source'));
  var baseUrl = "";
  var endUrl = "";
  var url = "";
  Logger.log("expr: " + expr);
  switch (source) {
    case 0: // codecogs.com (second)
      baseUrl = "https://latex.codecogs.com/gif.latex?\\dpi{200}&space;\\huge&space;";
      url = encodeURI(baseUrl + expr + endUrl);
      break;
    case 1: // roger's renderer (third)
      var randomEquationNumber = 48004; // protect user privacy
      Logger.log(randomEquationNumber);
      baseUrl = "http://rogercortesi.com/eqn/index.php?filename=tempimagedir%2Feqn" +
        randomEquationNumber + ".png&latextext=";
      endUrl = "&outtype=png&bgcolor=white&txcolor=black&res=350&transparent=1&antialias=1";
      UrlFetchApp.fetch(baseUrl + encodeURIComponent(expr) + endUrl);
      url = "http://rogercortesi.com/eqn/tempimagedir/eqn" + randomEquationNumber + ".png";
      break;
    case 2: // texrendr.com (best)
      baseUrl = "http://www.texrendr.com/cgi-bin/mathtex.cgi?\\dpi{400}";
      url = encodeURI(baseUrl) + encodeURIComponent(expr);
      Logger.log(url);
      break;
    case 3:
      baseUrl = "http://www.sciweavers.org/tex2img.php?eq=";
      endUrl = "&bc=White&fc=Black&im=png&fs=24&ff=modern&edit=0";
      url = encodeURI(baseUrl + encodeURIComponent(expr) + endUrl);
      Logger.log(url);
      break;
  }
  return url;
